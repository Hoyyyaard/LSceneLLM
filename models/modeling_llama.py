...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        if self.config.FLEX and layer_idx >= self.config.START_FLEX_LAYER:
            ...
            self.v_proj_hd = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        self._init_rope()
    ...
    def forward(
        ...
        **kwargs,
        ...
        if need_to_concat_hd_feature:
            ...
            attn_weights_select = nn.functional.dropout(attn_weights_select, p=self.attention_dropout, training=self.training)
