...
def init_slurm_distributed(local_rank):
    ...
    torch.distributed.init_process_group(
        backend=dist_backend, rank=(proc_id*num_gpus)+local_rank, world_size=ntasks * num_gpus, timeout=datetime.timedelta(seconds=18000)
    )
    # torch.distributed.barrier()
    print(">>>>> Success DDP >>>>>")
...
def all_gather_data(data):
    """
    Helper function to all_gather any type of data.
    """
    if isinstance(data, torch.Tensor):
        ...
        gathered_list = []
        for item in data:
            gathered_list.append(all_gather_data(item))
        return gathered_list
    elif isinstance(data, dict):
        return all_gather_dict(data)
    else:
        # Handle other data types (e.g., scalars)
        gathered_list = [None] * get_world_size()
        dist.all_gather_object(gathered_list, data)
        return gathered_list
    return data

def all_gather_dict(data: dict) -> dict:
    """
    Run all_gather on data which is a dictionary of various types.
    """
    assert isinstance(data, dict)
    
    gathered_dict = {}
    for item_key in data:
        gathered_dict[item_key] = all_gather_data(data[item_key])
    return gathered_dict